// Reconcile Notion (authoritative) → Supabase meeting_notes, keyed on notion_id.
// DEFAULT = dry-run. Pass --apply to upsert/delete.
//   node --env-file=.env.local --import tsx scripts/reconcile-meeting-notes.ts [--apply]
//
// Notes:
//  • Seed stored the 3 existing rows' notion_id with a 2-char garbage prefix
//    (34 chars vs the clean 32; prefix VARIES per row, so it can't be stripped).
//    Reconcile keys on the CLEAN dashless id → those 3 become orphans and are
//    re-inserted with canonical ids; +3 genuinely new meetings drifted into
//    Notion since the seed. Nothing joins on meeting_notes.notion_id, so the
//    3-row churn is safe.
//  • No client column — clientName lives inside the composite `name` title.
//    meeting_type CHECK: Annual Review/Follow-up/Phone Call/Video Call/Ad-hoc/
//    Onboarding — empty select → null (selN).
import { Client, isFullPage } from '@notionhq/client';
import type { PageObjectResponse } from '@notionhq/client/build/src/api-endpoints';
import pg from 'pg';

const APPLY = process.argv.includes('--apply');
const DB_ID = process.env.COMPANY_MEETING_NOTES_DB_ID!;

if (APPLY && process.env.DATA_SOURCE_MEETINGS === 'supabase') {
  console.error('✋ Refusing --apply: DATA_SOURCE_MEETINGS=supabase (post-cutover). Supabase is authoritative.');
  process.exit(1);
}
...
const COLS = ['name', 'meeting_date', 'meeting_type', 'next_review_date', 'notes', 'action_items', 'advisor'] as const;
type Col = typeof COLS[number];
type Rec = Record<Col, unknown>;
const DATE_COLS = new Set(['meeting_date', 'next_review_date']);

function recFromNotion(pg: PageObjectResponse): Rec {
  const p = pg.properties as P;
  return {
    name:             title(p, 'Name'),
    meeting_date:     dateOf(p, 'Meeting Date'),
    meeting_type:     selN(p, 'Meeting Type'),
    next_review_date: dateOf(p, 'Next Review Date'),
    notes:            rt(p, 'Notes'),
    action_items:     rt(p, 'Action Items'),
...
async function main() {
  ...
  const sbRows = (await db.query(
    `select id, notion_id, name, to_char(meeting_date,'YYYY-MM-DD') as meeting_date, meeting_type,
            to_char(next_review_date,'YYYY-MM-DD') as next_review_date, notes, action_items, advisor
     from meeting_notes`
  ...
  console.log(`\n── RECONCILE meeting_notes  (${APPLY ? 'APPLY' : 'DRY-RUN'}) ──`);
  console.log(`Notion: ${notionRecs.length}   Supabase: ${sbRows.length}`);
  console.log(`\n➕ INSERT: ${toInsert.length}`);
  toInsert.slice(0, 12).forEach(r => console.log(`   • [${r.rec.advisor}] ${(r.rec.name as string).slice(0, 44)}  ${r.rec.meeting_date}  ${r.rec.meeting_type ?? '-'}`));
  console.log(`\n✏️  UPDATE: ${toUpdate.length}`);
  toUpdate.slice(0, 12).forEach(u => console.log(`   • ${(u.rec.name as string).slice(0, 34)} | ${u.diffs.join(', ')}`));
  console.log(`\n🗑️  ORPHAN: ${orphans.length}`);
  orphans.slice(0, 12).forEach(r => console.log(`   • notion_id=${r.notion_id} (len ${(r.notion_id as string)?.length})  ${((r.name as string) || '').slice(0, 34)}`));
  ...
  for (const r of toInsert) { await db.query(`insert into meeting_notes (${cols.join(',')}) values (${ph$})`, [r.notion_id, ...COLS.map(c => r.rec[c])]); ins++; }
  for (const u of toUpdate) {
    const setSql = COLS.map((c, i) => `${c}=$${i + 2}`).join(', ');
    await db.query(`update meeting_notes set ${setSql} where notion_id=$1`, [u.notion_id, ...COLS.map(c => u.rec[c])]); upd++;
  }
  for (const r of orphans) { await db.query(`delete from meeting_notes where id=$1`, [r.id]); del++; }
  console.log(`\n✅ APPLIED — inserted ${ins}, updated ${upd}, deleted ${del} orphans.`);
  const total = (await db.query(`select count(*)::int n from meeting_notes`)).rows[0].n;
